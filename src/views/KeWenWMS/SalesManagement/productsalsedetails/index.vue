...
export default {
  ...
  created () {
    this.getList(this.$route.query.salseId, this.$route.query.orgId)
  },
  methods: {
    /** 查询销售出库单列表 */
    getList (salseId, orgId) {
      let that = this
      let token = getToken()
      let data = {}
      data.salseId = salseId
      data.orgId = orgId
      Vue.axios({
        method: 'post',
        ...
        if (response.data.code == 200) {
          that.setProductSalesDetails(response.data.data.wmTruckLoadingLineList)
        }
    ...
    setProductSalesDetails (res) {
      console.log(res)
      this.productSalesDetails = JSON.parse(JSON.stringify(res))
      console.log(this.productSalesDetails)
    },
    ...
    Delete () {
      ...
      let token = getToken()
      let data = {}
      data.salseId = this.productSalesDetails[0].salseId
    ...
    /**弃审之后跳转页面 */
    Goto () {
      this.$router.push({ path: '/KeWenWMS/SalesManagement/productsalse' })
    },
    //获取仓库信息
    getWarehouseList () {

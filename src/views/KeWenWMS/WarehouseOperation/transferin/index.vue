...
import { getList, MiscRcvTransDelete } from "@/api/WarehouseOperation/transferin"
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd () {
      this.$router.push({ path: '/KeWenWMS/WarehouseOperation/transferindetails', query: { row: { type: 'add' } } })
    },
    //点击编号跳转详情页面
    Goto (row) {
      row.type = 'detail'
      this.$router.push({ path: '/KeWenWMS/WarehouseOperation/transferindetails', query: { row: row } })
    },
    // 多选框选中数据

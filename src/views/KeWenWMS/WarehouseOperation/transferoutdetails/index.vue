<template>
  <div class="app-container">
    ...
    <el-form :model="form" ref="tableForm" :rules="tableRules" size="small">
      <el-table v-loading="loading" :data="form.wmTransferOutLineList">
        <el-table-column label="料号编码" align="center" prop="itemCode">
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.itemCode'"
              :rules="tableRules.itemCode"
            >
              <el-input
                :readonly="true"
                v-model="form.wmTransferOutLineList[$index].itemCode"
                placeholder="请选择物料编码"
        ...
        >
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.itemName'"
              :rules="tableRules.itemName"
            >
              <el-input
                v-model="form.wmTransferOutLineList[$index].itemName"
                placeholder="请输入物料名称"
              ></el-input>
            </el-form-item>
          </template>
        ...
        >
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.lotinfo'"
              :rules="tableRules.lotinfo"
            >
              <el-select
                v-model="form.wmTransferOutLineList[$index].lotinfo"
                placeholder="请选择"
        ...
        <el-table-column label="数量" align="center" prop="qty">
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.qty'"
              :rules="tableRules.qty"
            >
              <el-input-number
                size="medium"
                v-model="form.wmTransferOutLineList[$index].qty"
        ...
        <el-table-column label="单位" align="center" prop="unit">
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.unit'"
              :rules="tableRules.unit"
            >
              <el-input
                v-model="form.wmTransferOutLineList[$index].unit"
                placeholder="请输入单位"
              ></el-input>
            </el-form-item>
          </template>
        ...
        <el-table-column label="仓库" align="center" prop="attr1">
          <template slot-scope="{ row, $index }">
            <el-form-item
              :prop="'wmTransferOutLineList.' + $index + '.newattr1'"
              :rules="tableRules.newattr1"
            >
              <el-select
                v-model="form.wmTransferOutLineList[$index].newattr1"
                placeholder="请选择"
...
import { getDetail, miscRcvTrans, getBatch, addtInsert, listThread } from '@/api/WarehouseOperation/transferout'
  ...
  methods: {
    ...
    getList () {
      ...
      getDetail(params).then(res => {
        if (res.code == 200) {
          ...
          this.form.wmTransferOutLineList.forEach((item, index) => {
            this.transOptions.forEach(element => {
              if (item.attr1 == element.value) {
                this.$set(this.form.wmTransferOutLineList[index], 'newattr1', '')
                this.form.wmTransferOutLineList[index].newattr1 = element.label
              }
            })
          })
        }
    ...
    handleSubmit () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.form.wmTransferOutLineList.length == 0) {
    ...
    handleAddBtn () {
      ...
      let tempData = {}
      this.form.wmTransferOutLineList.push(tempData)
    },
    /**表格删除 */
    deleteRow (index) {
      this.form.wmTransferOutLineList.splice(index, 1)
    },
    /**查看物料详情 */
    ...
    selectOK (data, index) {
      this.$set(this.form.wmTransferOutLineList[index], 'itemCode', '')
      this.$set(this.form.wmTransferOutLineList[index], 'itemName', '')
      this.form.wmTransferOutLineList[index].itemCode = data.itemCode
      this.form.wmTransferOutLineList[index].itemName = data.itemName
      this.form.wmTransferOutLineList[index].itemId = data.itemId
      this.getBatch(this.form.wmTransferOutLineList[index].itemCode, index)
      ...
      getBatch(params).then(res => {
        if (res.code == 200) {
          ...
          if (res.rows.length == 0) {
            this.form.wmTransferOutLineList[index].lotinfo = ''
          }
    ...
    changeType (e, index) {
      ...
      this.$set(this.form.wmTransferOutLineList[index], 'storageLocationCode', '')
      this.form.wmTransferOutLineList[index].storageLocationCode = obj.value
    },
    changeTrans (e, index) {
      var obj = {}
      ...
      this.$set(this.form.wmTransferOutLineList[index], 'attr1', '')
      this.form.wmTransferOutLineList[index].attr1 = obj.value
    }
  }
}
</script>
